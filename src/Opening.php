...
class Opening
{
    ...
    {
        $schedules = array_filter($this->getSlots(), function (Schedule $slot) use ($time) {
            return $time->format('H:i') >= $slot->getStart()->format('H:i') &&
                $time->format('H:i') <= $slot->getEnd()->format('H:i');
        });
